...
describe("DocxFonts.embed edge shapes", () => {
  ...
  it("rejects bytes that are not a zip", async () => {
    await expect(embed(new Blob(["definitely not a zip archive"]), FAMS))
  ...
  it("regroups a stored (uncompressed) fontTable and defaults family/pitch", async () => {
    ...
    const out = await embed(blob, [
      { name: "Fam A", family: "swiss", pitch: "fixed",
    ...
    expect(ft).toContain('<w:font w:name="Fam A"><w:charset w:val="00"/>' +
    ...
    expect(ft).toContain('<w:font w:name="Fam B"><w:charset w:val="00"/>' +
