...
const rgbHex = (r: number, g: number, b: number) => "#" + [r, g, b].map(v =>
  Math.max(0, Math.min(255, Math.round(v))).toString(16).padStart(2, "0")).join("");
function mix(hex: string, other: string, k: number): string { // k = amount of `other`
...
const faceName = (key: unknown) => FACES[key as string] ? FACES[key as string]!.name
...
export const EngineFixture = {
  tints, faceName, fmtDate, PAGES, MARGINS, EMBEDDED, CUT_FILE, HL_COLORS,
};
