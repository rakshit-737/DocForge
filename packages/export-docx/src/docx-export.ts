...
export async function build(contentEl: HTMLElement, settings: DocxSettings, attachments?: unknown): Promise<Blob> {
  const D = window.docx;
  const {
    ...
    SectionType, TableLayoutType, SimpleField, Tab, TabStopType, NumberFormat,
  ...
  function collect(fn: () => void): any[] {
    targets.push(blocks);
    blocks = [];
    try { fn(); return blocks; } finally { blocks = targets.pop()!; }
  }
  ...
  const mkPara = (opts: any) => new Paragraph({
  ...
  const cellSafe = (arr: any[]) => {
    ...
    if (out[out.length - 1] instanceof Table) out.push(new Paragraph({ spacing: { after: 0 }, children: [] }));
    return out;
  };
  ...
  const para = (el: any, opts: any = {}) => new Paragraph({ children: runs(el, opts.fmt || {}), ...opts });
  ...
  function list(el: any, depth: number) {
    ...
    [...el.children].forEach(li => {
      ...
      const subs: any[] = [], extra: any[] = [];
      const clone = li.cloneNode(true);
      [...clone.children].forEach(c => {
        if (/^(ul|ol)$/i.test(c.tagName)) { subs.push(c); c.remove(); }
        else if (BLOCKY.test(c.tagName) || (c.classList && c.classList.contains("callout"))) { extra.push(c); c.remove(); }
      });
      blocks.push(new Paragraph({
  ...
  const CELL_ALIGN: Record<string, any> = { left: AlignmentType.LEFT, center: AlignmentType.CENTER, right: AlignmentType.RIGHT };
  ...
  function mdTable(el: any) {
    ...
    if (capEl) {
      ...
      blocks.push(new Paragraph({
        ...
        children: [
          new TextRun({ text: lbl ? lbl.textContent : "", bold: true, size: HP(9), color: hex(t.a800) }),
          new TextRun({ text: capEl.textContent.slice(lbl ? lbl.textContent.length : 0), size: HP(9), color: "5B6270" }),
        ],
      }));
    }
    const rows: any[] = [];
    ...
    const measured = (el.dataset.cols || "").split(",").map(Number).filter((n: number) => n > 0);
    const totalPx = measured.reduce((a: number, b: number) => a + b, 0);
    const totalTw = mm2t(pg.w - mg.l - mg.r);
    const colWidths = totalPx > 0
      ? measured.map((w: number) => Math.round((w / totalPx) * totalTw))
      : undefined;

    trs.forEach(tr => {
      const isHead = !!tr.querySelector("th");
      const cells = [...tr.children].map((td, i) => new TableCell({
  ...
  function boxTable(children: any[], { fill, border, dashed, height }: { fill: string; border: string; dashed?: boolean; height?: number }) {
  ...
  function callout(el: any) {
    ...
    const colors: Record<string, string> = { note: "2458C5", tip: "0E7A52", warning: "B26205", important: "BB2432" };
    const fills: Record<string, string> = { note: "EEF3FC", tip: "EBF7F1", warning: "FDF4E7", important: "FCEDEE" };
    ...
    if (bodyEl) inner.push(...collect(() => [...bodyEl.children].forEach((c: any) => walk(c, false))));
    const bs = (side: string) => ({ style: BorderStyle.SINGLE, size: side === "left" ? 20 : 2, color: side === "left" ? colors[type] : fills[type] });
    blocks.push(new Table({
      width: { size: 100, type: WidthType.PERCENTAGE },
      rows: [new TableRow({
        children: [new TableCell({
  ...
  function captionList(el: any) {
    ...
    if (title) blocks.push(new Paragraph({
      spacing: { after: 200 },
      keepNext: true,
      border: { bottom: { style: BorderStyle.SINGLE, size: 8, color: hex(t.a600), space: 3 } },
      children: [new TextRun({ text: title.textContent, bold: true, size: HP(18), font: f.head })],
    }));
  ...
  function walk(el: any, first: boolean): any {
    ...
    if (el.classList) {
      ...
      if (alClass) {
        ...
        } as Record<string, any>)[alClass];
        if (al) return withCtx({ align: al }, () => [...el.children].forEach((c: any) => walk(c, false)));
      }
    }
    ...
    if (tag === "blockquote") {
      return withCtx({
        ...
        if (c.nodeType === 1) walk(c, false);
          ...
          blocks.push(mkPara({ children: [new TextRun({ text: c.textContent.replace(/\s+/g, " "), italics: true, color: "3D434D" })] }));
        }
      }));
    }
    ...
    if (el.children && el.children.length) return [...el.children].forEach((c: any) => walk(c, false));
    // Anything else that carries text still reaches the page rather than vanishing.
    if (el.textContent && el.textContent.trim()) blocks.push(mkPara({ children: runs(el, ctx.fmt) }));
  }
  ...
  const NO_BORDERS = { top: NONE_B, bottom: NONE_B, left: NONE_B, right: NONE_B, insideHorizontal: NONE_B, insideVertical: NONE_B };
  const ZERO_M = { top: 0, bottom: 0, left: 0, right: 0 };
  /* An empty Paragraph is a whole line box and would blow out an EXACT-height band. */
  const tiny = () => new Paragraph({ spacing: { before: 0, after: 0, line: 20, lineRule: "exact" }, children: [] });
  const bandCell = (fill: string) => new TableCell({ shading: { fill }, margins: ZERO_M, children: [tiny()] });
  const bleedTable = (rows: any[]) => new Table({
    width: { size: 100, type: WidthType.PERCENTAGE },
  ...
  if (settings.cover) {
    ...
    const padL = mm2t(spine ? 10 : 24), padR = mm2t(24);
    const cp = (o: any) => new Paragraph({ indent: { left: padL, right: padR }, alignment: al, ...o });
    ...
    const accentRule = new Table({
      ...
      margins: ZERO_M, borders: NO_BORDERS,
      rows: [new TableRow({
    ...
    const content = [
      cp({
        ...
        children: settings.kicker ? [new TextRun({ text: settings.kicker.toUpperCase(), bold: true, size: HP(10), color: hex(t.a700), characterSpacing: centred ? 60 : 36, font: f.head })] : [],
      }),
      cp({
        spacing: { after: 260 },
    ...
    if (spine) {
      ...
      coverBlocks.push(bleedTable([new TableRow({
        height: { value: mm2t(pg.h), rule: HeightRule.EXACT },
        children: [
          new TableCell({ shading: { fill: hex(t.a800) }, margins: ZERO_M, width: { size: mm2t(9), type: WidthType.DXA }, children: [tiny()] }),
          new TableCell({ margins: ZERO_M, verticalAlign: VerticalAlign.TOP, children: content }),
        ],
      })]));
    } else {
      const rows: any[] = [];
      if (!centred) rows.push(new TableRow({ height: { value: mm2t(BAND_TOP), rule: HeightRule.EXACT }, children: [bandCell(topFill)] }));
      rows.push(new TableRow({
        height: { value: mm2t(pg.h - (centred ? 0 : BAND_TOP) - BAND_BOT), rule: HeightRule.EXACT },
        children: [new TableCell({ margins: ZERO_M, verticalAlign: VerticalAlign.TOP, children: content })],
      }));
      if (BAND_BOT) rows.push(new TableRow({ height: { value: mm2t(BAND_BOT), rule: HeightRule.EXACT }, children: [bandCell(hex(t.a200))] }));
  ...
  const frontHeaderChildren = settings.header ? [new Paragraph({
    style: "DFHeader",
    children: [new TextRun({ text: (settings.title || "").toUpperCase(), characterSpacing: 26 })],
  })] : [];
  ...
  const footerChildren = settings.pageNums ? [new Paragraph({
    alignment: AlignmentType.CENTER,
    children: [pgRun(["Page "]), pgRun([PageNumber.CURRENT]), pgRun([" of "]),
      pgRun([PageNumber.TOTAL_PAGES_IN_SECTION])],
  })] : [];
  // Front matter shows a bare roman numeral, matching the PDF.
  const frontFooterChildren = settings.pageNums ? [new Paragraph({
    alignment: AlignmentType.CENTER,
  ...
  const bodyMargin = {
    top: mm2t(mg.t), right: mm2t(mg.r), bottom: mm2t(mg.b), left: mm2t(mg.l),
    header: mm2t(12), footer: mm2t(12), gutter: 0,
  };
  ...
  contentEl.querySelectorAll<HTMLElement>("span.dfspan[data-font]").forEach(s => usedFonts.add(s.dataset.font!));
  ...
  const lineTw = ({ "1": 240, "1.15": 276, "1.5": 360, "2": 480 } as Record<string, number | undefined>)[settings.lineSpacing as string];
  ...
  const doc = new Document({
    ...
    styles: {
      default: {
        ...
        heading4: { run: { font: f.head, size: HP(11), bold: true, italics: true, color: "333A45" }, paragraph: { spacing: { before: 43, after: 88 } } },
        heading5: { run: { font: f.head, size: HP(10.5), bold: true, color: "333A45" }, paragraph: { spacing: { before: 160, after: 70 } } },
        heading6: { run: { font: f.head, size: HP(10), bold: true, color: "4A5160" }, paragraph: { spacing: { before: 140, after: 60 } } },
    ...
    const BW = ({ fine: 6, medium: 12, bold: 18 } as Record<string, number | undefined>)[settings.borderWeight as string] || 12;
    ...
    } as Record<string, { style: any; size: number; color?: string } | undefined>)[settings.borderStyle as string];
    if (PB) PB.color = BC;
