/* ============================================================
   mathml-omml.js — MathML → OMML (Office Math)
   ============================================================
   ...
   Self-contained: its own tolerant XML reader, no DOM, no dependencies. Runs
   as a browser global (it is inlined into the single-file app) and as a node
   module, so the QA harness can drive it head-less.

     MathmlOmml.mmlToOmml(mathml)     -> "<m:oMath …>…</m:oMath>" | null
     MathmlOmml.texToOmml(tex, disp)  -> the same, via the global `katex` | null
     MathmlOmml.oMathPara(omml, jc)   -> display wrapper for a whole paragraph

   Both converters return null rather than guess when they cannot make sense of
   the input, so the caller can fall back to printing the source: a silently
   wrong equation is worse than no equation.

   MIT licensed, like the rest of DocForge. Written against the OMML shapes in
   ECMA-376 part 1 §22.1 and against what KaTeX 0.18 actually emits.
   ============================================================ */
"use strict";
...
const MathmlOmml = (() => {
  ...
  const STRIP = /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u200b-\u200f\u2061-\u2064\ufeff]/g;
  const clean = s => String(s == null ? "" : s).replace(STRIP, "");
  ...
  const local = n => { const i = n.indexOf(":"); return i < 0 ? n : n.slice(i + 1); };
  /* MathML elements that never have content; tolerated without a closing slash. */
  const EMPTY_EL = { mspace: 1, mprescripts: 1, none: 1, mglyph: 1, mline: 1 };

  function parseXml(src) {
    const doc = { name: "#doc", attrs: {}, kids: [] };
    const stack = [doc];
    const top = () => stack[stack.length - 1];
    const text = t => { if (t) top().kids.push({ name: "#text", attrs: {}, kids: [], text: t }); };
    const n = src.length;
    let i = 0;
    ...
    while (i < n) {
      ...
      if (!head) continue;
      const el = { name: local(head[1]), attrs: {}, kids: [] };
      const ar = /([^\s=/>]+)\s*=\s*("([^"]*)"|'([^']*)'|([^\s"'>]+))/g;
      let a;
  ...
  /* ---------- tree helpers ---------- */
  const TRANSPARENT = {                    // pure grouping / styling, no OMML of its own
  ...
  const DROP = { annotation: 1, "annotation-xml": 1, maligngroup: 1, malignmark: 1 };
  const TOKEN = { mi: 1, mn: 1, mo: 1, mtext: 1, ms: 1 };

  const isEl = nd => nd && nd.name !== "#text";
  function textOf(nd) {
    if (!nd) return "";
  ...
  const hasEl = nd => nd.kids.some(isEl);
  ...
  function kidsOf(nd) {
    ...
    for (let i = 0; i < nd.kids.length; i++) {
      const k = nd.kids[i];
      if (k.name === "#text") { if (/\S/.test(k.text)) out.push(k); continue; }
  ...
  const BAR_TOP = "‾¯\u0305―—";
  const BAR_BOT = "_▁\u0332";
  const GROUP_TOP = { "⏞": 1, "⎴": 1, "⏜": 1, "︷": 1, "{": 1, "⎰": 1 };
  const GROUP_BOT = { "⏟": 1, "⎵": 1, "⏝": 1, "︸": 1, "}": 1, "⎱": 1 };
  ...
  function render(atoms) {
    let out = "", pend = null, ppr = "";
    const flush = () => {
      if (pend === null) return;
  ...
  const val = (tag, v) => "<m:" + tag + " m:val=\"" + escA(v) + "\"/>";

  function slot(tag, nodes, ctx, bareWhenEmpty) {
  ...
  function naryOf(nd) {
    ...
    const k = kidsOf(nd);
    let base = null, sub = null, sup = null, loc = "";
    switch (nd.name) {
  ...
  const SCRIPTY = { msub: 1, msup: 1, msubsup: 1, munder: 1, mover: 1, munderover: 1 };
  const isApply = nd =>
  ...
  /* ---------- the converter ---------- */
  function emitList(nodes, ctx) { return render(atomsOf(nodes, ctx)); }

  function atomsOf(nodes, ctx) {
  ...
  function emitNary(n, operand, ctx) {
  ...
  function emitFenced(nd, ctx) {
    const fix = c => (FENCE_FIX[c] || c);
  ...
  function emitMulti(k, ctx) {
    ...
    const live = nd => (nd && nd.name !== "none" ? nd : null);
    let core;
  ...
  function emitTable(nd, rows, ctx) {
  ...
  function texToOmml(tex, display) {
  ...
  return { mmlToOmml: mmlToOmml, texToOmml: texToOmml, oMathPara: oMathPara, NS: M_NS };
})();

if (typeof module !== "undefined" && module.exports) module.exports = MathmlOmml;
