/* ============================================================
   file-import.js — more formats in, Markdown out
...
const FileImport = (() => {
  ...
  const mdCell = t => String(t ?? "").replace(/\s+/g, " ").replace(/\|/g, "\\|").trim();

  function mdTable(rows) {
    if (!rows.length) return "";
    const width = Math.max(...rows.map(r => r.length));
    const pad = r => { const o = r.slice(); while (o.length < width) o.push(""); return o; };
    const line = r => "| " + pad(r).map(mdCell).join(" | ") + " |";
  ...
  function csv(text) {
    ...
    const counts = [[",", 0], [";", 0], ["\t", 0]].map(([ch]) => [ch, first.split(ch).length - 1]);
  ...
  async function xlsx(buf) {
    ...
    const relMap = {};
    local(rels, "Relationship").forEach(r => { relMap[r.getAttribute("Id")] = r.getAttribute("Target"); });
  ...
  const runText = p => local(p, "t").map(t => t.textContent).join("");
  ...
  async function pptx(buf) {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      if (z.has(slideRels)) {
        ...
        if (note) {
          const notePath = "ppt/" + note.getAttribute("Target").replace(/^(\.\.\/)+/, "").replace(/^\.?\//, "");
          if (z.has(notePath)) {
  ...
  async function epub(buf, htmlToMd) {
    const z = zipOpen(buf);
    ...
    const manifest = {};
    local(opf, "item").forEach(it => { manifest[it.getAttribute("id")] = it.getAttribute("href"); });
    const order = local(opf, "itemref").map(ir => manifest[ir.getAttribute("idref")]).filter(Boolean);
  ...
  return { csv, xlsx, pptx, epub, ipynb };
})();
