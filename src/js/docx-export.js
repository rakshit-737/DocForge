...
const DocxExport = (() => {
  ...
  async function build(contentEl, settings, attachments) {
    const D = window.docx;
    const {
    ...
    const mkPara = opts => new Paragraph({
    ...
    const CELL_ALIGN = { left: AlignmentType.LEFT, center: AlignmentType.CENTER, right: AlignmentType.RIGHT };
    ...
    function mdTable(el) {
      ...
      const measured = (el.dataset.cols || "").split(",").map(Number).filter(n => n > 0);
      const totalPx = measured.reduce((a, b) => a + b, 0);
      const totalTw = mm2t(pg.w - mg.l - mg.r);
      const colWidths = totalPx > 0
        ? measured.map(w => Math.round((w / totalPx) * totalTw))
        : undefined;
    ...
    function callout(el) {
      ...
      if (bodyEl) inner.push(...collect(() => [...bodyEl.children].forEach(c => walk(c, false))));
    ...
    function captionList(el) {
      ...
      el.querySelectorAll("nav.lst > a").forEach(a => {
    ...
    function walk(el, first) {
      ...
      if (el.children && el.children.length) return [...el.children].forEach(c => walk(c, false));
    ...
    const bandCell = fill => new TableCell({ shading: { fill }, margins: ZERO_M, children: [tiny()] });
    const bleedTable = rows => new Table({
      width: { size: 100, type: WidthType.PERCENTAGE },
    ...
    if (settings.cover) {
      ...
      const cp = o => new Paragraph({ indent: { left: padL, right: padR }, alignment: al, ...o });
