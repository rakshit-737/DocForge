...
- **Editor comforts** — a command palette with every action searchable by name (Ctrl+K), outline navigator (☰ above the preview), find & replace (Ctrl+F / Ctrl+H), pasted Word/web content auto-converted to Markdown, a gentle structure checker that flags anything that would break the export, a drop zone that names what dropping a file will do, and a light/dark switch for the app chrome (the document always prints on white)
