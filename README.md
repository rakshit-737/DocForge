...
- **Edit PDFs in place** — the layout you received is the layout you keep: **double-click any printed line to rewrite it** (it reappears at the same position and size, set in the nearest standard face), add text anywhere, white-out or highlight regions, stamp images; export rebuilds from the *original bytes* with only your edits drawn on top. Two honest limits: text doesn't reflow (a line is rewritten, not re-typeset), and a rewritten line's original characters remain in the file's hidden text layer beneath the cover — don't use it to redact secrets
