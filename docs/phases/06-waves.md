...
| Landscape pages (§8.2) | **Shipped** | one `Engine.pageSpec()` answers "which sheet is this?" for both formats — the CSS asks the printer for `A4 landscape`, Word gets the upright sheet plus `w:orient` and turns it itself (handing it the turned sheet as well would turn it back). 6 engine tests, 2 export tests, studio probe 5/5, a golden case on the widest corpus document, and a real-Word print measured at 842×596 pt against the PDF's 842×595. Per-SECTION landscape needs a dialect marker and is not ours to decide |
